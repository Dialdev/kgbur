...
global $arTheme;
$bHideOnNarrow = $arTheme['BIGBANNER_HIDEONNARROW'] === 'Y';
$slideshowSpeed = abs(intval($arTheme['BIGBANNER_SLIDESSHOWSPEED']));
$animationSpeed = abs(intval($arTheme['BIGBANNER_ANIMATIONSPEED']));
$bAnimation = ($slideshowSpeed && strlen($arTheme['BIGBANNER_ANIMATIONTYPE']));
if($arTheme['BIGBANNER_ANIMATIONTYPE'] === 'FADE'){
	$animationType = 'fade';
}
else{
	$animationType = 'slide';
	$animationDirection = 'horizontal';
	if($arTheme['BIGBANNER_ANIMATIONTYPE'] === 'SLIDE_VERTICAL'){
		$animationDirection = 'vertical';
	}
}
?>
<div class="top-slider flexslider unstyled<?=($bHideOnNarrow ? ' hidden_narrow' : '')?><?=($animationDirection == 'vertical' ? ' vertical' : '')?>" data-plugin-options='{"directionNav": true, "controlNav": true, <?=($bAnimation ? '"slideshow": true,' : '"slideshow": false,')?> <?=($animationType ? '"animation": "'.$animationType.'",' : '')?> <?=($animationDirection ? '"direction": "'.$animationDirection.'",' : '')?> <?=($slideshowSpeed >= 0 ? '"slideshowSpeed": '.$slideshowSpeed.',' : '')?> <?=($animationSpeed >= 0 ? '"animationSpeed": '.$animationSpeed.',' : '')?> "animationLoop": true}'>
	<ul class="slides">
		<?foreach($arResult["ITEMS"] as $arItem):?>
			...
			?>
			<li class="kgbslider" id="<?=$this->GetEditAreaId($arItem['ID']);?>" style="background:url('<?=$img?>')  no-repeat;">
