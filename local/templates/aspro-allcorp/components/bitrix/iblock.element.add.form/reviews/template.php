...
<p><font color="green" class="notetext">Ваш отзыв успешно добавлен</font></p>
<?endif?>
...
<table class="data-table" style="width: 100%;display: grid;justify-content: center;">
		<thead>
			<tr>
				<td colspan="2">&nbsp;</td>
			</tr>
					...
					<td>
						...
						switch ($INPUT_TYPE):
							...
							case "T":
								...
								?>
						<textarea placeholder="Отзыв*" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="5" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
								<?
								}
							break;
							...
							case "N":
								...
								?>
								<input placeholder="Ваше Имя*" type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?=$value?>" /><br /><?
		...
		<tfoot>
			<tr>
				<td colspan="2">
					<input style="margin-top: 10px;" class="btn btn-primary" type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
...
.form-body {
    padding: 30px 85px;
    background: url(/local/templates/aspro-allcorp/images/background.png) #eaf0f2 0 0 no-repeat;
