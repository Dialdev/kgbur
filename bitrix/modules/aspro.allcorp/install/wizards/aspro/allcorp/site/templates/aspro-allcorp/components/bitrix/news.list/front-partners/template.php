...
global $arTheme;
$slideshowSpeed = abs(intval($arTheme['PARTNERSBANNER_SLIDESSHOWSPEED']));
$animationSpeed = abs(intval($arTheme['PARTNERSBANNER_ANIMATIONSPEED']));
$bAnimation = (bool)$slideshowSpeed;
?>
<div class="partners hidden-sm hidden-xs">
	<div class="flexslider unstyled" data-plugin-options='{"directionNav": false, "controlNav": false, <?=($bAnimation ? '"slideshow": true,' : '"slideshow": false,')?> <?=($slideshowSpeed >= 0 ? '"slideshowSpeed": '.$slideshowSpeed.',' : '')?> <?=($animationSpeed >= 0 ? '"animationSpeed": '.$animationSpeed.',' : '')?> "animationLoop": true}'>
		<ul class="slides">
			<li>
				<div class="row">
					<?$i = 1;?>
