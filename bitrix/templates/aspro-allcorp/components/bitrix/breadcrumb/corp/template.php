...
for($index = 0, $itemSize = count($arResult); $index < $itemSize; ++$index){
	...
	if(strlen($arResult[$index]["LINK"]) && $arResult[$index]['LINK'] != GetPagePath() && $arResult[$index]['LINK']."index.php" != GetPagePath())
		...
		$strReturn .= '<li class="active" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" style="display:none;" href="'.(CMain::IsHTTPS() ? 'https' : 'http').'://'.$_SERVER['SERVER_NAME'].$arResult[$index]['LINK'].'"></a><span itemprop="title">'.$title.'</span></li>';
		break;
	}
}
