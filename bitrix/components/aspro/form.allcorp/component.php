			...
			if( count( $arResult["FORM_ERRORS"] ) <= 0 ){
				//if( check_bitrix_sessid() ){
					...
					if( $RESULT_ID = $el->Add( $arFields ) ){
						...
						$arDataTrigger = json_decode((isset($_REQUEST["data-trigger"]) ? $_REQUEST["data-trigger"] : '{}'), true);
						$url = (((isset($arDataTrigger['data-page-url']) && $arDataTrigger['data-page-url'])) ? $arDataTrigger['data-page-url'] : '');
						...
						$arEventFields = array(
							...
							"PAGE_LINK" => ((CSite::inDir(SITE_DIR.'ajax/') || CSite::inDir(SITE_DIR.'form/')) ? ($url ? $url : (isset($_SERVER["HTTP_REFERER"]) && $_SERVER["HTTP_REFERER"] ? $_SERVER["HTTP_REFERER"] : $APPLICATION->GetCurPage())) : $APPLICATION->GetCurPage()),
						);
							...
							foreach( $arResult["QUESTIONS"] as $FIELD_CODE => $arQuestion ){
								if($FIELD_CODE !== $antiSpamHiddenFieldCode){
									...
									elseif($arQuestion["FIELD_TYPE"] == "file"){
										...
										/*if($arQuestion["FIELD_TYPE"] == "html"){
											$arEventFields[$FIELD_CODE] = $arQuestion["VALUE"]['TEXT'];
										}
										else{*/
						...
						CEvent::Send( $arMessAdmin["EVENT_NAME"], SITE_ID, $arEventFields, "Y", $arMessAdmin["ID"] );
